#!/usr/bin/env python3
"""Parse inert console and collector units in a disposable root; never start them."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="macserver-dashboard-units-") as temporary:
        ...
        fixtures = {"usr/bin/python3": "#!/bin/sh\nexit 1\n"}
        for name, content in fixtures.items():
            path = root / name; path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(content); path.chmod(0o755)
        for name in ("sysinit", "basic", "shutdown", "network", "network-online", "multi-user", "timers"):
            for directory in (system, user):
                (directory / f"{name}.target").write_text("[Unit]\nDescription=Parser fixture only\n")
        for name in ("docker", "tailscaled", "systemd-user-sessions", "getty@tty1"):
    ...
    print("PASS: console and collector unit parsers with disposable stubs; not runtime qualification")
